...
def getJsonReponse(cookieJar, proxy):
    url = "https://twitter.com/search?q=%20%23ibukotabaru%20since%3A2018-08-26%20until%3A2019-08-27&src=typed_query"
...
# while active:
html = getJsonReponse(cookieJar, None)
...
for date in list_date:
    for page in tweepy.Cursor(api.search, since=date["start_date"], lang="id", q=hashtag).pages():
        for items in page:
            user_info = api.get_user(items._json["user"]["id"])

            user_name = user_info._json["name"]
            screen_name = user_info._json["screen_name"]
            print(screen_name)
            location = user_info._json["location"]
            result_analyze = analyze(items._json["text"])
            date_time_str = items._json["created_at"]
            created_at = date_time_str
            csvWriter.writerow([created_at, user_name, screen_name, location, result_analyze["compound"], result_analyze["pos"], result_analyze["neu"], result_analyze["neg"], result_analyze["clean_tweet"].encode('utf-8')])